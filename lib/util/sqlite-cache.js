...
async function createTables() {
  ...
  db.exec(`CREATE UNIQUE INDEX IF NOT EXISTS idx_service_hash ON cache(service, hash)`);
...
export async function upsertCachedMagnet(record, options = {}) {
  ...
  try {
    ...
    stmt.run({
      ...
      createdAt: now, // Preserved on conflict, only updated fields change
      updatedAt: now,
      expiresAt
    });
    const duration = Date.now() - startTime;
...
export async function upsertCachedMagnets(records, options = {}) {
  ...
  try {
    ...
    // Use transaction for batched writes (better performance and atomicity)
