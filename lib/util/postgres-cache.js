...
export async function upsertCachedMagnet(record, options = {}) {
  ...
  if (upsertQueue.length >= UPSERT_QUEUE_MAX) {
    ...
    // Reject the promises of entries about to be dropped before slicing
    const dropCount = Math.floor(UPSERT_QUEUE_MAX * 0.1);
    for (let i = 0; i < dropCount && i < upsertQueue.length; i++) {
      upsertQueue[i].resolve(false);
    }
    // Drop oldest 10% of queue
    upsertQueue = upsertQueue.slice(dropCount);
  }
