...
function coreTitleTokens(str) {
  ...
  for (const t of all) {
    if (isSeasonEpisodeToken(t) || isResolutionToken(t)) break;
    // Only treat year as a stop token AFTER we've seen at least one alphabetic token
    if (isYearToken(t) && foundAlpha) break;
    if (/[a-z]/i.test(t)) foundAlpha = true;
    core.push(t);
  }
...
export function matchesCandidateTitle(candidate, ctx, opts = {}) {
  ...
  for (const raw of values) {
    ...
    if ((q.length <= 2 && coverage < 1) || (q.length > 2 && coverage < 0.7)) continue;
