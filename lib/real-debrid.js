  ...
  try {
    ...
    const cachedExternalTorrents = externalTorrents.filter(t => {
      ...
      if (episodeInfo && !t.episodeFileHint) {
        ...
        if (isPack) {
          // Keep cached season packs but mark them for later episode file inspection
          t.needsEpisodeInspection = true;
          console.log(`[${LOG_PREFIX}] Keeping cached season pack for inspection: ${t.Title || t.name}`);
        }
      }
      return true;
    });
    ...
    if (episodeInfo) {
        allResults = allResults.filter(item => {
            ...
            if (isPackByPtt) {
                console.log(`[${LOG_PREFIX}] Keeping cached pack in results (PTT): ${item.name}`);
                return true; // Cached packs are kept - episode file will be resolved at playback
            }

            // PTT thinks it's a single episode. Double check for episode ranges like E01-E10.
            if (/\bE\d{2,3}[-~_]E?\d{2,3}\b/i.test(title)) {
                console.log(`[${LOG_PREFIX}] Keeping cached pack in results (range regex): ${item.name}`);
                return true; // Cached packs are kept - episode file will be resolved at playback
            }
      ...
      setImmediate(async () => {
        try {
            ...
            try {
                ...
                // Clear the set so the outer finally block doesn't re-delete the same torrents
