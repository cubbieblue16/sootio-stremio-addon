...
export function normalizeInfoHash(value) {
    ...
    // 40-char hex hash - return as-is
    if (/^[a-f0-9]{40}$/.test(candidate)) {
        return candidate;
    }
    // 32-char Base32 hash - convert to hex
    if (/^[a-z2-7]{32}$/.test(candidate)) {
        const hex = base32ToHex(candidate);
        return hex ? hex.toLowerCase() : null;
    }

    const fromMagnet = getHashFromMagnet(raw);
    if (fromMagnet) {
        const lower = fromMagnet.toLowerCase();
        if (/^[a-f0-9]{40}$/.test(lower)) {
            return lower;
        }
        if (/^[a-z2-7]{32}$/.test(lower)) {
            const hex = base32ToHex(lower);
            return hex ? hex.toLowerCase() : null;
        }
    }
...
export function sizeToBytes(sizeStr) {
    if (!sizeStr) return 0;
    const units = { 'TB': 1099511627776, 'GB': 1073741824, 'MB': 1048576, 'KB': 1024, 'B': 1 };
...
export function calculateTorrentPriority(torrent) {
    ...
    // Penalties
    if (/\bcam\b/i.test(name) || /\bts\b/i.test(name) || /\bscreener\b/i.test(name) || name.includes('hdrip')) priorityScore -= 100;
