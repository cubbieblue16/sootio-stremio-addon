 ...
 */
export function makeRequest(url, options = {}, redirectCount = 0) {
            ...
            req = protocol.request(requestOptions, (res) => {
                ...
                if ((res.statusCode === 301 || res.statusCode === 302 || res.statusCode === 307 || res.statusCode === 308) &&
                    ...
                    if (redirectCount >= MAX_REDIRECTS) {
                        res.destroy();
                        finish(reject, new Error(`Maximum redirect limit (${MAX_REDIRECTS}) exceeded for ${url}`));
                        return;
                    }
                    const redirectUrl = new URL(res.headers.location, url).toString();
                    console.log(`Following redirect from ${url} to ${redirectUrl} (${redirectCount + 1}/${MAX_REDIRECTS})`);
                    res.destroy();
                    finish(resolve, makeRequest(redirectUrl, options, redirectCount + 1));
                    return;
                }
