...
export async function getCachedTorrents(provider, type, id, config, searchFn) {
  ...
  if (combinedResults.length === 0) {
    ...
    // Set ForceFullScrape on a shallow copy to avoid mutating the shared config object
    // across concurrent requests. searchFn is a closure over config, so we must mutate
    // the original reference, but we save and restore immediately around the await.
    const prevForceFullScrape = config.ForceFullScrape;
    config.ForceFullScrape = true;
    let rawFreshResults;
    try {
      rawFreshResults = await searchFn();
    } finally {
